			     ...
			     bool tx_pause, bool rx_pause)
{
	...
	if (!(bp->caps & MACB_CAPS_MACB_IS_EMAC)) {
		...
		/* EtherCAT mode: do not enable queue interrupts (master polls). */
		if (!get_ecdev(bp)) {
			for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue) {
...
static int macb_phylink_connect(struct macb *bp)
{
	...
	/* EtherCAT: ecdev_open() drives ndo_open() directly, without the
	 * RTNL lock the normal netdev open path holds. phylink_start()
	 * asserts RTNL, so take it here in EtherCAT mode (cf. stmmac). */
	if (get_ecdev(bp)) {
		rtnl_lock();
	}
...
static void macb_tx_unmap(struct macb *bp, struct macb_tx_skb *tx_skb, int budget)
{
	...
	if (tx_skb->skb) {
		/* EtherCAT mode: the master owns and reuses a persistent ring of
		 * TX skbs (ec_device_tx_data); unmap but never free them.
		 */
		if (!get_ecdev(bp)) {
...
static int macb_rx_poll(struct napi_struct *napi, int budget)
{
	...
	/* EtherCAT mode polls via ec_poll(); NAPI must never run here. */
	if (get_ecdev(bp)) {
		return 0;
	}
 ...
 *
 * Called by the EtherCAT master once per cycle (in the master's realtime
 * thread). Drives TX completion and RX drain directly over every queue,
 * bypassing NAPI/IRQ. The RX path (mog_rx == gem_rx in EtherCAT mode) hands
 * frames to the master via ecdev_receive(). Cannot fail.
 *
 * NOTE: only the GEM RX path (gem_rx) carries the EtherCAT hook; RP1 is GEM,
 * so mog_rx resolves to gem_rx. Non-GEM macb is not yet EtherCAT-enabled.
 */
...
static int macb_pad_and_fcs(struct sk_buff **skb, struct net_device *ndev)
{
	...
	/* EtherCAT mode: this GEM still needs a software FCS for EtherCAT
	 * frames (raw L2, ip_summed == CHECKSUM_NONE) because TX csum offload
	 * (TXCOEN) is on and the MAC then omits the FCS for non-offloaded
	 * frames -- exactly as in generic mode, which works. But the EtherCAT
	 * master reuses a single persistent TX skb and rewrites skb->len each
	 * cycle (ec_device_send) WITHOUT resetting the tail pointer, so a plain
	 * skb_put() of the FCS walks ->tail further past ->end every cycle and
	 * eventually triggers skb_over_panic() (BUG at skb_put), crashing the
	 * box from ec_master's send path.
	 *
	 * Handle EtherCAT explicitly: re-sync the tail pointer to skb->len, then
	 * append the 4-byte FCS in place. Never pad, clone, reallocate or free
	 * here -- the master owns the skb (its descriptor ring points at it) and
	 * EtherCAT frames are always >= ETH_ZLEN with ample tailroom.
	 */
	if (get_ecdev(bp)) {
		skb_set_tail_pointer(*skb, (*skb)->len);
		if (likely(skb_tailroom(*skb) >= ETH_FCS_LEN)) {
...
static int macb_open(struct net_device *dev)
{
	...
	/* EtherCAT: skip NIC PTP/PHC bring-up. The netdev is never
	 * registered in EtherCAT mode, so gem_ptp_init()'s posix-clock
	 * registration WARNs and fails (-ENOENT); IgH uses bus DC, not
	 * the MAC's hardware clock, so this path is pure noise here. */
...
static int macb_close(struct net_device *dev)
{
	...
	/* EtherCAT: ndo_stop() runs without RTNL here (see macb_open). */
	if (get_ecdev(bp)) {
		rtnl_lock();
	}
...
static int macb_init(struct platform_device *pdev)
{
	...
	for (hw_q = 0, q = 0; hw_q < MACB_MAX_QUEUES; ++hw_q) {
		...
		/* EtherCAT mode: the master polls via ec_poll(); request no IRQ
		 * so the ISR never runs against the (unenabled) NAPI contexts.
		 */
		if (!get_ecdev(bp)) {
...
static int macb_probe(struct platform_device *pdev)
{
	...
	/* Offer the device to the EtherCAT master before IP-specific init,
	 * because macb_init() requests the queue IRQs and we must gate that
	 * on EtherCAT mode (the master polls; no IRQ is requested).
	 */
	...
	if (get_ecdev(bp)) {
		/* EtherCAT mode: let the master open the device (calls ndo_open
		 * = macb_open for hardware bring-up). Do not register a netdev.
		 */
...
static int __maybe_unused macb_resume(struct device *dev)
{
	...
	if (bp->ptp_info && !get_ecdev(bp))	/* EtherCAT: no NIC PTP, see macb_open */
