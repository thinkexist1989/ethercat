...
static void e100_watchdog_impl(struct nic *nic)
{
...
process_skb:
	...
	if (nic->ecdev) {
		...
		if (rx->prev->skb) {
			struct rfd *prev_rfd = (struct rfd *) rx->prev->skb->data;
			put_unaligned_le32(rx->dma_addr, &prev_rfd->link);
			dma_sync_single_for_device(&nic->pdev->dev, rx->prev->dma_addr,
					sizeof(struct rfd), DMA_TO_DEVICE);
		}
	} else {
		rx->skb = NULL;
	}
