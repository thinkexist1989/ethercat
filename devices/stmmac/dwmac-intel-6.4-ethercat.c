...
#include <linux/pci.h>
#include <linux/dmi.h>
#include "dwmac-intel-6.4-ethercat.h"
#include "dwmac4-6.4-ethercat.h"
#include "stmmac-6.4-ethercat.h"
#include "stmmac_ptp-6.4-ethercat.h"
...
static int __init dwmac_init(void)
{
	int ret;
	ret = stmmac_init();
	if (ret) {
	...
	ret = pci_register_driver(&intel_eth_pci_driver);
	if (ret) {
		stmmac_exit();
	}
	return ret;
...
static void __exit dwmac_exit(void)
{
	pci_unregister_driver(&intel_eth_pci_driver);
	stmmac_exit();
}
